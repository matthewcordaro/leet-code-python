...
class Solution:
  ...
  def topKFrequent(self, nums: List[int], k: int) -> List[int]:
    ...
    for num in nums: freq[num] -= 1  # go negative for heapq
    heap = []
    for num, quantity in freq.items(): heapq.heappush(heap, (quantity, num))
    nums = []  # repurpose nums for the solution
    while (k:=k-1) >= 0: nums.append(heapq.heappop(heap)[1])
    return nums
